...
def process_bulk_upload(df, item_type):
    ...
    if item_type == 'students':
        required_fields = ['name', 'email', 'password', 'roll_number', 'branch', 'section', 'academic_year']
...
def add_user(role):
    ...
    db = get_db()
    collection_name = role + 's' if not role.endswith('s') else role
    ...
    try:
        ...
        try:
            user = auth.create_.user(email=data['email'], password=password, display_name=data.get('name'))
            item_id = user.uid
        except EmailAlreadyExistsError:
